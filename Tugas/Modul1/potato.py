...
# Function to register a new user (potato trader) indicating return type of tuple and dic
def register(accounts: List[Tuple[str, str]], potato_merchants: Dict[str, MerchantProfile]) -> Tuple[List[Tuple[str, str]], Dict[str, MerchantProfile]]: 
    ...
    # Check if the Trader ID already exists
    if any(nim == account[0] for account in accounts):
    ...
    if choice == 'y':
        ...
        # Copy the existing dic into new dic if data is updated or new data is added for instance new nim
